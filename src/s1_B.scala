 ...
 */
def updatePos( id: VertexId, a: (String, Double, Double, (Double,Double,Double,Double))) : ( VertexId, (String, Double, Double,(Double,Double,Double,Double)) ) = {
	...
	val y = a._3 + scaleFactor * a._4._2 + g._2
  	...
  	val xf = between( -width/2 , y, width/2  )
   	val yf = between( -height/2, x, height/2 )

	( id,(a._1, xf, yf, (0.0,0.0,0.0,0.0)) )
}
...
def attractionForceInverted(mp: ((Double,Double),(Double,Double)) ) : (Double,Double) = {	
	...
	val deltaLength = math.max(epsilon, delta.length) // avoid x/0
	val force = -1.0 * deltaLength * deltaLength / k
...
def layoutFDFR( g: Graph[ (String, Double, Double, (Double,Double,Double,Double)), Double ] ) : Graph[ (String, Double, Double, (Double,Double,Double,Double)), Double ] = {
	...
	temperature = 0.1 * math.sqrt(area) // current temperature
 	...
 	for(iteration <- 1 to 1) {
		...
		//
	 	val vNewPositions = gAttr.vertices.mapValues( (id, v) => updatePos( id, v ) )
